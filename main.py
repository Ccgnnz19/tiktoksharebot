...
class TikTok:
    ...
    def update_title(self):
        ...
        while self.added < self.amount:
            ...
            os.system(
                "title Tqkn's TikTok SharesBot - Added: {self.added} Shares/{self.amount} Shares "
                '({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
                '{threading.active_count()} ^| Time Remaining: {time_remaining}'
            )
            sleep(0.2)
        os.system(
            "title Tqkn's TikTok SharesBot - Added: {self.added}/{self.amount} "
            '({round(((self.added / self.amount) * 100), 3)}%) ^| Active Threads: '
    ...
    def bot(self):
        ...
        if self.added > 0:
            print("{self.added} Shares have been added!")
        ...
        headers = {
            ...
            'x-common-params-v2': 'version_code=16.6.5&app_name=musical_ly&channel=App%20Store&devi'
                                  'ce_id={device_id}&aid=1233&os_version=13.5.1&device_platform=ip'
